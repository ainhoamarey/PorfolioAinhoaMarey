...
if (mobileWindowsBtn && mobileStartMenu) {
  ...
  mobileStartMenu.querySelectorAll('.mobile-start-app').forEach(app => {
    app.addEventListener('click', () => {
      ...
      if (win) openWindow(win, { fullscreen: true });
      closeMobileStartMenu();
    });
  });
}
...
// =============================
// TRADUCCIONES (claves usadas en tu HTML con data-translate*)
// =============================
const translations = {
  es: {
    projects: "Proyectos",
    ...
    proj3_desc: "Página de centro de estetica con galeria de fotos ",
    ...
    proj6_desc: "pagina con menu e Iconos",
    proj7_desc: "pagina web con canciones de queen",
    tech_html: "HTML",
    tech_css: "CSS",
    tech_js: "JavaScript",
    tech_php: "PHP",
  ...
  en: {
    ...
    nameTitle: "Ainhoa Marey",
    cvSubtitle: "Multiplatform developer & web design",
    experienceTitle: "Experience",
    exp1_desc: "Facilitating playful and educational activities with minors.",
    exp2_desc: "Workshops and games organization at city events.",
    exp3_desc: "Care and pedagogical support for early childhood.",
    exp4_desc: "Handcrafted products and customer service.",
    exp5_place: "Child care between 5 and 13 years old",
    exp5_desc: "Homework supervision, snacks and recreational activities.",
    exp6_desc: "Support in washing, coloring and customer service.",
    exp7_desc: "Sorting and distributing food to people in need.",
    cvNote: "Click \"Download PDF\" to view the full resume",
    backToPreview: "Back to preview",
    aboutName: "Ainhoa Marey",
    aboutTagline: "Multiplatform developer eager to turn ideas into unique digital experiences",
    aboutParagraph: "I am a developer in training with a solid foundation in multiplatform application development and a strong motivation to keep growing in the tech world. I consider myself creative, resourceful and highly adaptable, qualities strengthened in previous jobs where communication, teamwork and organization were key. I’m currently focusing on designing and building efficient, functional applications while learning new technologies and agile methods. I enjoy projects that are challenging and allow me to deliver useful, innovative solutions.",
    timelineTitle: "My Timeline",
    tl1_desc: "Learning Java, databases, UI design, web development and more.",
    tl2_desc: "Activity coordination, leadership, communication and teamwork.",
    ...
    tl5_desc: "Key complementary training in youth leadership.",
    ...
    eclipseDesc: "IDE focused on Java",
    netbeansDesc: "Java development IDE",
    ...
    springBootDesc: "Spring extension for rapid configuration",
    languagesTitle: "Languages",
    languagesDesc: "Programming and markup languages",
    javaDesc: "Object-oriented, cross-platform language",
    htmlDesc: "Markup language for webpages",
    cssDesc: "Style language for web design",
    ...
    spanish: "Español",
    english: "English",
    proj1_desc: "Personal portfolio styled like a Windows desktop",
    proj2_desc: "Online store with cart and product management",
    proj3_desc: "A beauty center website with photo gallery",
    ...
    proj7_desc: "Web page with Queen’s songs",
    tech_html: "HTML",
    tech_css: "CSS",
    tech_js: "JavaScript",
    tech_php: "PHP",
...
function setLang(lang) {
  ...
  changeLanguage(lang); // fuerza actualización de otros textos
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setTimeout(() => { changeLanguage(savedLanguage); }, 100);
...
function initializeApp() {
  ...
  // Doble clic iconos escritorio (desktop)
  ...
  // Doble clic menú inicio (desktop)
...
// =============================
// OPEN WINDOW (con soporte fullscreen en móvil)
// =============================
function openWindow(windowType, opts = {}) {
    ...
    // Si ya hay una instancia visible, solo traer al frente
    if (windowEl.style.display !== 'none') {
      bringToFront(windowEl);
      return;
    }
  
    // Crear nueva instancia clonando la plantilla oculta
    const newWindow = windowEl.cloneNode(true);
    const uniqueId = `${windowType}-${++windowCounter}`;
    newWindow.id = uniqueId;
  
    // Posición y tamaño por defecto (modo ventana)
    const windowWidth = 800;
    const windowHeight = 600;
    const desktopRect = document.getElementById('desktop').getBoundingClientRect();
    const left = Math.max(0, (desktopRect.width  - windowWidth)  / 2);
    const top  = Math.max(0, (desktopRect.height - windowHeight) / 2);
    ...
    // ⬇️ Pantalla completa en móvil o si se pide explícitamente
    if (isMobile || (opts && opts.fullscreen)) {
      newWindow.classList.add('fullscreen');
    ...
    if (windowType === 'cv') setupCVFunctionalityForWindow(newWindow);
    if (windowType === 'skills') setupSkillsNavigationForWindow(newWindow);
  }
  

// =============================
// VENTANA - Eventos / arrastre / tamaño
// =============================
function setupWindowEvents(windowEl, windowType) {
  ...
  // 👉 Arrastrar / redimensionar SOLO en ESCRITORIO
  if (!isMobile) {
...
// Actualiza títulos de todas las ventanas y botones de taskbar
...
function setupMobileExperience() {
  ...
  // 👉 En móvil, abrir con UN toque y a PANTALLA COMPLETA
  document.querySelectorAll('.mobile-app').forEach(app => {
    app.addEventListener('click', () => {
      const windowType = app.getAttribute('data-window');
      if (windowType) openWindow(windowType, { fullscreen: true });
    }, { passive: true });
  });
